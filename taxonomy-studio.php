<?php
/** Studio profile archive. */
defined( 'ABSPATH' ) || exit;
get_header();
$term  = get_queried_object();
$image = $term instanceof WP_Term ? bmd_get_term_image( $term, 'large' ) : '';
$letter = $term instanceof WP_Term ? ( function_exists( 'mb_substr' ) ? mb_substr( $term->name, 0, 1 ) : substr( $term->name, 0, 1 ) ) : 'S';
?>
<section class="bmd-profile-hero">
	<div class="bmd-container bmd-profile-grid">
		<a class="bmd-back-link" href="<?php echo esc_url( bmd_page_url_by_template( 'template-studio.php', '/studio/' ) ); ?>">← <?php esc_html_e( 'กลับหน้ารวม', 'bossmaster-display' ); ?></a>
		...
		<div class="bmd-profile-copy">
			<p class="bmd-eyebrow"><?php esc_html_e( 'STUDIO PROFILE', 'bossmaster-display' ); ?></p>
